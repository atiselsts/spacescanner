...
from psutil_local._error import Error, NoSuchProcess, AccessDenied, TimeoutExpired
from psutil_local._common import cached_property
from psutil_local._compat import property, callable, defaultdict, namedtuple
from psutil_local._compat import (wraps as _wraps,
                            PY3 as _PY3)
from psutil_local._common import (deprecated as _deprecated,
                            nt_disk_iostat as _nt_disk_iostat,
                            nt_net_iostat as _nt_net_iostat,
                            nt_sysmeminfo as _nt_sysmeminfo)

from psutil_local._common import (STATUS_RUNNING,
                            STATUS_IDLE,
                            STATUS_SLEEPING,
                            STATUS_DISK_SLEEP,
                            STATUS_STOPPED,
...
from psutil_local._common import (CONN_ESTABLISHED,
                            CONN_SYN_SENT,
                            CONN_SYN_RECV,
                            CONN_FIN_WAIT1,
                            CONN_FIN_WAIT2,
...
if sys.platform.startswith("linux"):
    import psutil_local._pslinux as _psplatform
    from psutil_local._pslinux import (phymem_buffers,
                                 cached_phymem)

    from psutil_local._pslinux import (IOPRIO_CLASS_NONE,
                                 IOPRIO_CLASS_RT,
                                 IOPRIO_CLASS_BE,
                                 IOPRIO_CLASS_IDLE)
    # Linux >= 2.6.36
    if _psplatform.HAS_PRLIMIT:
        from psutil_local._pslinux import (RLIM_INFINITY,
                                     RLIMIT_AS,
                                     RLIMIT_CORE,
                                     RLIMIT_CPU,
                                     RLIMIT_DATA,
    ...
    import psutil_local._psmswindows as _psplatform
    from psutil_local._psmswindows import (ABOVE_NORMAL_PRIORITY_CLASS,
    ...
    import psutil_local._psosx as _psplatform

elif sys.platform.startswith("freebsd"):
    import psutil_local._psbsd as _psplatform

elif sys.platform.startswith("sunos"):
    import psutil_local._pssunos as _psplatform
    from psutil_local._pssunos import (CONN_IDLE,
                                 CONN_BOUND)
...
class Process(object):
    ...
    def get_cpu_percent(self, interval=0.1):
        ...
        Examples:
          ...
          >>> p = psutil_local.Process(os.getpid())
          >>> # blocking
...
class Popen(Process):
    ...
    property and methods of psutil_local.Process class in a single interface:

      >>> import psutil
      >>> from subprocess import PIPE
      >>> p = psutil_local.Popen(["/usr/bin/python", "-c", "print 'hi'"], stdout=PIPE)
      >>> p.name
      'python'
      >>> p.uids
    ...
    and wait(), psutil_local.Process implementation takes precedence.
...
def pid_exists(pid):
    ...
    This is faster than doing "pid in psutil_local.get_pid_list()" and
    should be preferred.
    """
...
def cpu_percent(interval=0.1, percpu=False):
    ...
    Examples:
      ...
      >>> psutil_local.cpu_percent(interval=1)
      2.0
      >>>
      >>> # blocking, per-cpu
      >>> psutil_local.cpu_percent(interval=1, percpu=True)
      [2.0, 1.0]
      >>>
      >>> # non-blocking (percentage since last call)
      >>> psutil_local.cpu_percent(interval=0)
      2.9
      >>>
    """
...
def get_boot_time():
    ...
    This is also available as psutil_local.BOOT_TIME.
    """
...
def phymem_usage():
    ...
    Deprecated by psutil_local.virtual_memory().
    """
    mem = virtual_memory()
    return _nt_sysmeminfo(mem.total, mem.used, mem.free, mem.percent)


@_deprecated("psutil_local.swap_memory()")
def virtmem_usage():
    return swap_memory()


@_deprecated("psutil_local.phymem_usage().free")
def avail_phymem():
    return phymem_usage().free


@_deprecated("psutil_local.phymem_usage().used")
def used_phymem():
    return phymem_usage().used


@_deprecated("psutil_local.virtmem_usage().total")
def total_virtmem():
    return virtmem_usage().total


@_deprecated("psutil_local.virtmem_usage().used")
def used_virtmem():
    return virtmem_usage().used


@_deprecated("psutil_local.virtmem_usage().free")
def avail_virtmem():
    return virtmem_usage().free


@_deprecated("psutil_local.net_io_counters()")
...
def test():
    ...
    """
    import datetime
    from psutil_local._compat import print_
